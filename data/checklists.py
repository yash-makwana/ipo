...
BUSINESS_OVERVIEW_CHECKLIST = [
    ...
    # New sophisticated checks
    "SEASONALITY_DISCLOSURE",
    "PROPOSED_CAPACITY_BASIS",
    "PATENT_VALIDITY_RISK",
    "FOREX_HEDGING_POLICY"
]
...
MANDATORY_BUSINESS_OVERVIEW = [
    ...
    "LEASE_EXPIRY_RISK",
    "PRODUCT_WISE_DOMESTIC_EXPORT_SPLIT",
    "YOY_REVENUE_VARIANCE_ANALYSIS",
    "SCRAP_WASTAGE_MANUFACTURING",
    "RD_BRANDING_EXPENDITURE_TREND",
    "UNNAMED_FOREIGN_ENTITIES",
    "COMPETITION_INDUSTRY_REVENUE_RATIONALE",
    # New sophisticated mandatory checks (Jan 2026)
    "SEASONALITY_DISCLOSURE",
    "PROPOSED_CAPACITY_BASIS",
    "PATENT_VALIDITY_RISK",
    "FOREX_HEDGING_POLICY"
...
NSE_ISSUES = [
    ...
    {
        "id": "PRODUCT_WISE_DOMESTIC_EXPORT_SPLIT",
        "category": "Financial",
        "severity": "Critical - Valuation",
        "metric_label": "Product Revenue Split",
        "intent": "Revenue Clarity",
        "primary_evidence_regex": r"(revenue\s+from\s+operations|product\s+wise\s+revenue|sales\s+bifurcation|revenue\s+break)",
        "consolidated_template": "On page no. {anchor_pages}, it is mentioned revenue from operations. Kindly provide product-wise bifurcation of domestic and export sales in absolute terms (₹ lakhs/crores) and percentage (%) for the past three financial years and stub period. Provide revised draft along with confirmation that the same shall be updated in prospectus.",
        "mandatory": True
    },
    {
        "id": "YOY_REVENUE_VARIANCE_ANALYSIS",
        "category": "Financial",
        "severity": "Critical - Valuation",
        "metric_label": "Revenue Variance",
        "intent": "Trend Analysis",
        "primary_evidence_regex": r"(revenue.*declined|sales.*decreased|turnover.*dropped|revenue.*reduced|declined.*from.*₹|decreased.*from.*₹)",
        "consolidated_template": "On page no. {anchor_pages}, it has been observed that revenue/sales have declined between financial years. Kindly provide detailed reasons for the decline including: (a) product-wise analysis of decline; (b) impact of market conditions; (c) management actions taken; and (d) confirmation of impact on future operations. Provide revised draft along with confirmation that the same shall be updated in prospectus.",
        "mandatory": True
    },
    {
        "id": "SCRAP_WASTAGE_MANUFACTURING",
        "category": "Operational",
        "severity": "Material",
        "metric_label": "Manufacturing Efficiency",
        "intent": "Operational Metrics",
        "primary_evidence_regex": r"(manufacturing\s+process|production\s+process|processing\s+of|manufacturing\s+operations)",
        "consolidated_template": "On page no. {anchor_pages}, it is mentioned manufacturing process. Kindly provide details of scrap and wastage in the manufacturing process including: (a) scrap percentage and wastage rates; (b) yield loss analysis; (c) scrap disposal mechanism and revenue recovery, if any. Provide revised draft along with confirmation that the same shall be updated in prospectus.",
        "mandatory": True
    },
    {
        "id": "RD_BRANDING_EXPENDITURE_TREND",
        "category": "Financial",
        "severity": "Material",
        "metric_label": "R&D Investment",
        "intent": "Capability Assessment",
        "primary_evidence_regex": r"(r&d|research\s+and\s+development|innovation|product\s+development|research\s+activities)",
        "consolidated_template": "On page no. {anchor_pages}, it is mentioned R&D/product development. Kindly provide: (a) R&D expenditure in absolute terms (₹ lakhs) and as % of revenue for past 3 financial years and stub period; (b) branding and marketing expenditure for the same period; (c) 3-year trend analysis showing YoY changes. Provide revised draft along with confirmation that the same shall be updated in prospectus.",
        "mandatory": True
    },
    {
        "id": "PAGE_TOTAL_ROW_MISSING",
        "category": "Hygiene",
        "severity": "Hygiene",
        "metric_label": "Table Formatting",
        "intent": "Data Completeness",
        "primary_evidence_regex": r"(table|tabular\s+format|following\s+table)",
        "consolidated_template": "On page no. {anchor_pages}, it has been observed that tables do not include TOTAL rows for numerical columns. Kindly review all tables in the Business Overview chapter and include TOTAL rows where applicable to enable easy reconciliation. Provide revised draft along with confirmation that the same shall be updated in prospectus.",
        "mandatory": False
    },
    {
        "id": "UNNAMED_FOREIGN_ENTITIES",
        "category": "Compliance",
        "severity": "Major",
        "metric_label": "Entity Disclosure",
        "intent": "Transparency",
        "primary_evidence_regex": r"(chinese\s+(?:distributor|supplier|customer|manufacturer)|foreign\s+(?:distributor|supplier|customer)|international\s+(?:supplier|vendor|partner)|overseas\s+(?:supplier|vendor))",
        "consolidated_template": "On page no. {anchor_pages}, it is mentioned {snippet}. The Company mentions foreign business partner(s) without providing complete names and details. Kindly provide: (a) complete name and registration details of such entity; (b) nature and terms of business arrangement; (c) dependency analysis and alternate arrangements, if any. Provide revised draft along with confirmation that the same shall be updated in prospectus.",
        "mandatory": True
    },
    {
        "id": "NVH_PRODUCT_APPLICATION_DETAILS",
        "category": "Operational",
        "severity": "Material",
        "metric_label": "Product Details",
        "intent": "Product Clarity",
        "primary_evidence_regex": r"(NVH|NVAH|noise.*vibration.*harshness|acoustic)",
        "consolidated_template": "On page no. {anchor_pages}, it is mentioned NVH/NVAH. Kindly include detailed product usage in NVH applications including: (a) specific product lines used in NVH applications; (b) end-use industry mapping (automotive, industrial, etc.); (c) revenue contribution from NVH applications. Provide revised draft along with confirmation that the same shall be updated in prospectus.",
        "mandatory": False
    },
    {
        "id": "COMPETITION_INDUSTRY_REVENUE_RATIONALE",
        "category": "Strategic",
        "severity": "Material",
        "metric_label": "Competition Disclosure",
        "intent": "Market Positioning",
        "primary_evidence_regex": r"(competition\s+section|competitors|industry\s+revenue|market\s+share|competitive\s+landscape)",
        "consolidated_template": "On page no. {anchor_pages}, it is mentioned competition/industry. Kindly provide: (a) industry-wise revenue from operations split showing Company's positioning; (b) rationale for competition section inclusion/exclusion; (c) peer comparison with market leaders, if material. Provide revised draft along with confirmation that the same shall be updated in prospectus.",
        "mandatory": True
    },
    {
    ...
    # ========================================================================
    # NEW SOPHISTICATED CHECKS - Advanced Compliance (Jan 2026)
    # ========================================================================
    
    {
        "id": "SEASONALITY_DISCLOSURE",
        "category": "Forensic",
        "severity": "Critical - Valuation",
        "metric_label": "Seasonality Impact",
        "intent": "Revenue Predictability",
        "primary_evidence_regex": r"(seasonal|cyclical|monsoon|quarterly\s+fluctuation|peak\s+season|agricultural|automotive\s+cycle)",
        "consolidated_template": "On page no. {anchor_pages}, it is mentioned {snippet}. Kindly provide a quarterly revenue trend for the last 3 years (₹ lakhs and % of annual revenue) to demonstrate the impact of seasonality/cyclicality on the business operations. Further, include a specific Risk Factor addressing revenue concentration in specific quarters and its impact on working capital requirements. Provide revised draft along with confirmation that the same shall be updated in prospectus.",
        "relocation_hint": "Business Overview / MD&A",
        "mandatory": True
    },
    
    {
        "id": "PROPOSED_CAPACITY_BASIS",
        "category": "Forensic",
        "severity": "Critical - Valuation",
        "metric_label": "Future Capacity Estimation",
        "intent": "Projection Integrity",
        "primary_evidence_regex": r"(proposed\s+capacity|expanded\s+capacity|future\s+production|target\s+output|post-expansion|commissioning|installed\s+capacity)",
        "consolidated_template": "On page no. {anchor_pages}, proposed/expanded capacity is mentioned. Kindly clarify by: (a) providing the basis of this estimation (e.g., machinery manufacturer specifications vs management estimate vs technical feasibility report); (b) specifying the 'Assumed Utilization Rate' used for financial projections (% of installed capacity); (c) reconciling this with Objects of the Offer capital expenditure; and (d) including a Risk Factor for delay in achieving target utilization. Provide revised draft along with confirmation that the same shall be updated in prospectus.",
        "relocation_hint": "Objects of the Offer / Business Overview",
        "mandatory": True
    },
    
    {
        "id": "PATENT_VALIDITY_RISK",
        "category": "Legal",
        "severity": "Critical - Operational",
        "metric_label": "IP Expiration Risk",
        "intent": "Technology Obsolescence",
        "primary_evidence_regex": r"(patent|proprietary\s+technology|license\s+validity|technical\s+collaboration|technology\s+transfer|intellectual\s+property|exclusive\s+license)",
        "consolidated_template": "On page no. {anchor_pages}, technical collaboration/patents/proprietary technology are mentioned. Kindly provide: (a) a comprehensive table of all valid patents, trademarks, and technical licenses with their specific expiration dates; (b) the financial impact (₹ lakhs and % of revenue) attributable to patented/licensed products; (c) confirmation of exclusivity terms and termination clauses; and (d) a specific Risk Factor addressing the impact of technology obsolescence, license termination, or patent expiry on business continuity. Provide revised draft along with confirmation that the same shall be updated in prospectus.",
        "relocation_hint": "Business Overview / Risk Factors / Legal",
        "mandatory": True
    },
    
    {
        "id": "FOREX_HEDGING_POLICY",
        "category": "Forensic",
        "severity": "Critical - Valuation",
        "metric_label": "Currency Risk Management",
        "intent": "Financial Risk Assessment",
        "primary_evidence_regex": r"(foreign\s+currency|exchange\s+rate|euro|dollar|forex|hedging|unhedged|import|export|overseas|foreign\s+supplier|Landi\s+Renzo|Italy|international\s+transaction)",
        "consolidated_template": "On page no. {anchor_pages}, foreign currency transactions/imports/exports are mentioned. Kindly provide: (a) the Company's formal Hedging Policy with Board approval details; (b) a comprehensive table of 'Unhedged Foreign Currency Exposure' as of the latest financial period (currency-wise in ₹ lakhs and as % of total liabilities); (c) sensitivity analysis showing ₹ impact on PAT for a 5% and 10% movement in exchange rates; (d) details of any forward contracts, currency swaps, or natural hedges in place; and (e) historical forex gain/loss for the past 3 years (₹ lakhs and % of EBITDA). Provide revised draft along with confirmation that the same shall be updated in prospectus.",
        "relocation_hint": "Business Overview / Financial Risk / MD&A",
        "mandatory": True
    }
]
