...
class BusinessModelNormalizer:
    ...
    MANUFACTURING_RELEVANT = {
        'CAPACITY_CERTIFICATION', 'CAPACITY_UTILIZATION', 'UTILITIES_DISCLOSURE',
        'OPERATIONAL_SCRAP', 'OWN_VS_CONTRACTUAL_MANUFACTURING', 
        'PROCESS_FLOW_ALIGNMENT', 'QUALITY_CONTROL_DISCLOSURE',
        'RAW_MATERIAL_PRICE_VOLATILITY', 'SUPPLIER_CONCENTRATION',
        'POLLUTION_CONTROL_COMPLIANCE', 'INVENTORY_MANAGEMENT'
    }
    
    SERVICES_RELEVANT = {
        'ATTRITION_RATE', 'CUSTOMER_CHURN_REPEAT', 'KEY_MAN_DEPENDENCY_RISK',
        'MANAGEMENT_EXPERIENCE_INDIVIDUAL', 'RD_DISCLOSURE',
        'BRANDING_INVESTMENT_QUANTIFICATION', 'IT_SECURITY_DISCLOSURE',
    ...
    TRADING_RELEVANT = {
        ...
        'WAREHOUSING_CAPACITY_LIMITS'
    }
    
    # Universal queries (apply to all business types)
    UNIVERSAL_QUERIES = {
        'REVENUE_MODEL_EXPLANATION', 'REVENUE_DISCLOSURE', 'GEOGRAPHIC_REVENUE_SPLIT',
        'DOMESTIC_EXPORT_RECON', 'CSR_DISCLOSURE',
        'LEASE_STATUS_MANDATORY', 'INSURANCE_DISCLOSURE',
        ...
        'RELATED_PARTY_TRANSACTIONS', 'PROMOTER_GROUP_DISCLOSURE',
        'IT_DATA_SECURITY_DISCLOSURE', 'MATERIAL_AGREEMENTS_DISCLOSURE'
    }
...
class DisclosureStatusClassifier:
    ...
    MISSING_INDICATORS = [
        ...
        'absence of', 'missing', 'omitted', 'kindly confirm whether',
        'no information', 'not available', 'disclose particulars'
    ]
    
    INADEQUATE_INDICATORS = [
        'insufficient', 'inadequate', 'vague', 'unclear', 'generic',
        'kindly elaborate', 'kindly provide details', 'kindly clarify',
        'disclosure is provided only in descriptive', 'absolute figures',
        'percentage terms', 'not clearly disclosed', 'specific details',
        'breakdown has not been provided', 'further detail'
    ]
    
    INCONSISTENT_INDICATORS = [
        'contradiction', 'inconsistent', 'mismatch', 'discrepancy',
        'reconcile', 'conflicting', 'differs from', 'differing figures',
        'kindly clarify the discrepancy'
    ]
    ...
    def classify(self, query_text: str) -> str:
        ...
        # Check for inadequate (often higher detail than just missing)
...
class MaterialityScorer:
    ...
    def _extract_amounts(self, text: str) -> List[float]:
        """Extract ₹ amounts from text including variety of patterns"""
        amounts = []
        
        # Pattern 1: ₹X crores, ₹X lakhs, ₹X Cr, ₹X L, etc.
        patterns = [
            (r'(?:₹|Rs\.?)\s*(\d+(?:,\d+)?(?:\.\d+)?)\s*(?:crore|crores|Cr|Cr\.)', 1.0),
            (r'(?:₹|Rs\.?)\s*(\d+(?:,\d+)?(?:\.\d+)?)\s*(?:lakh|lakhs|L|L\.)', 0.01),
            (r'(?:₹|Rs\.?)\s*(\d+(?:,\d+)?(?:\.\d+)?)\s*(?:million|millions|Mn|Mn\.)', 0.1),
            (r'(?:₹|Rs\.?)\s*(\d+(?:,\d+)?(?:\.\d+)?)\s*(?:billion|billions|Bn|Bn\.)', 100.0)
        ]
        
        for p, multiplier in patterns:
...
class QueryCollisionResolver:
    ...
    TOPIC_KEYWORDS = {
        'LITIGATION': ['litigation', 'legal', 'lawsuit', 'dispute', 'case', 'pending', 'court'],
        'REVENUE': ['revenue', 'sales', 'turnover', 'income', 'topline'],
        'CAPACITY': ['capacity', 'utilization', 'production', 'unit', 'manufacturing'],
        'ATTRITION': ['attrition', 'employee', 'turnover', 'retention', 'human resource', 'kmp'],
        'SUPPLIER': ['supplier', 'vendor', 'procurement', 'sourcing'],
        'CUSTOMER': ['customer', 'client', 'distributor', 'concentration'],
        'INSURANCE': ['insurance', 'coverage', 'policy', 'insured'],
        'LEASE': ['lease', 'rental', 'property', 'premises', 'agreement'],
        'RD': ['research', 'development', 'R&D', 'innovation'],
        'UTILITIES': ['utility', 'power', 'water', 'electricity'],
        'IT_SECURITY': ['IT', 'security', 'data', 'breach', 'disaster'],
        'CSR': ['CSR', 'social responsibility', 'expenditure'],
        'SUBSIDIARY': ['subsidiary', 'associate', 'group company'],
        'DIRECTOR': ['director', 'board', 'management', 'kmp'],
    }
    ...
    def _combine_observations(self, observations: List[str], topic: str) -> str:
        ...
        # Clean and deduplicate sentences across all observations
        all_sentences = []
        for obs in observations:
            # Simple sentence split (handles common NSE patterns)
            parts = re.split(r'(?<=[\.\!\?])\s+', obs)
        ...
        for sent in all_sentences:
            # Create a "core" version for deduping (remove page refs and numeric values)
            core = re.sub(r'Page\s+\d+|₹[\d,]+|\d+%|\d+', '', sent, flags=re.IGNORECASE).lower().strip()
            if core not in seen_core:
                unique_sentences.append(sent)
                seen_core.add(core)
        
        # Re-join into a coherent multi-part query
        if len(unique_sentences) > 4:
            # If too many, take the first two and last two (intro and conclusion)
            unique_sentences = unique_sentences[:2] + unique_sentences[-2:]
            
        combined = " ".join(unique_sentences)
        
        # Ensure it starts with a clean "On Page X of the DRHP..." style if possible
        # or has a cohesive flow
        return combined
