...
class ContradictionDetector:
    ...
    # Numeric patterns with context (Improved with more variations)
    NUMERIC_PATTERNS = {
        'REVENUE': r'(?:revenue|turnover|sales|total\s+income).*?₹?\s*([\d,]+(?:\.\d+)?)\s*(crore|cr|lakh|million|billion)',
        'EMPLOYEES': r'(?:employees?|workforce|personnel|headcount).*?([\d,]+)',
        'CAPACITY': r'(?:capacity|production|output).*?([\d,]+(?:\.\d+)?)\s*(units?|tonnes?|MT|KL|pieces?|kg|liters?)',
        'DEALERS': r'(?:dealers?|distributors?|channels?|outlets?).*?([\d,]+)',
        'PRODUCTS': r'(?:products?|SKUs?|variants?).*?([\d,]+)',
        'FACILITIES': r'(?:facilities|plants?|units?|locations?|warehouses?).*?([\d,]+)',
        'CUSTOMERS': r'(?:customers?|clients?|buyers?).*?([\d,]+)',
        'SUPPLIERS': r'(?:suppliers?|vendors?|sellers?).*?([\d,]+)',
    }
    
    # Entity relationship patterns
    RELATIONSHIP_PATTERNS = {
        'SUBSIDIARY': r'subsidiary|subsidiaries|wholly\s+owned',
        'GROUP_COMPANY': r'group\s+compan(?:y|ies)|promoter\s+group',
        'ASSOCIATE': r'associate(?:d)?(?:\s+compan(?:y|ies))?',
        'JOINT_VENTURE': r'joint\s+venture|consortium',
        'RELATED_PARTY': r'related\s+part(?:y|ies)|key\s+management\s+personnel',
    }
    
    # Business activity patterns
    ACTIVITY_PATTERNS = {
        'MANUFACTURING': r'manufactur(?:e|es|ing|er|ing\s+process)',
        'TRADING': r'trad(?:e|es|ing|er|resell(?:ing|er))',
        'DISTRIBUTION': r'distribut(?:e|es|ing|or|wholesal(?:e|er))',
        'SERVICES': r'service(?:s)?|consulting|advisory',
    }
    
    TOLERANCE_PCT = 1.0  # Reduced to 1% tolerance for higher accuracy
...
class DraftingHygieneScanner:
    ...
    def _check_section_references(self, page: int, text: str) -> List[Dict]:
        ...
        # Find section references like "refer to Chapter X"
        ref_pattern = r'(?:see|refer\s+to|mentioned\s+in)\s+(?:Section|Chapter|Page)\s+([0-9\.]+)'
        refs = re.findall(ref_pattern, text, re.IGNORECASE)
        
        for ref in refs:
            # If reference is "0" or "0.0" or very high, might be a placeholder
            if ref in ['0', '0.0', 'XX']:
                queries.append({
                    'type': 'hygiene',
                    'page': str(page),
                    'observation': (
                        f"On page no. {page}, a placeholder or invalid section reference '{ref}' was detected. "
                        f"Kindly ensure all internal cross-references are correctly updated. "
                        f"Provide revised draft."
                    ),
                    'severity': 'Hygiene',
                    'category': 'Drafting Quality',
                    'issue_id': 'INVALID_REFERENCE',
                    'regulation_ref': 'SEBI Circular on DRHP Format'
                })
        
        return queries
        
    def _check_date_consistency(self, page: int, text: str) -> List[Dict]:
        """Check for inconsistent date formats (e.g. 31/03/24 and 31-Mar-24)"""
        queries = []
        
        # Pattern 1: DD/MM/YY or DD.MM.YY
        slash_dates = re.findall(r'\b\d{1,2}[\/\.]\d{1,2}[\/\.]\d{2,4}\b', text)
        
        # Pattern 2: DD-Mon-YY
        dash_dates = re.findall(r'\b\d{1,2}-(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)-\d{2,4}\b', text, re.IGNORECASE)
        
        if len(slash_dates) > 2 and len(dash_dates) > 2:
            queries.append({
                'type': 'hygiene',
                'page': str(page),
                'observation': (
                    f"On page no. {page}, multiple date formats (e.g., '{slash_dates[0]}' and '{dash_dates[0]}') "
                    f"were detected. Kindly ensure consistent date formatting throughout the document. "
                    f"Provide revised draft."
                ),
                'severity': 'Hygiene',
                'category': 'Drafting Quality',
                'issue_id': 'INCONSISTENT_DATE_FORMAT',
                'regulation_ref': 'SEBI Circular on DRHP Format'
            })
    ...
    def scan(self, pages_dict: Dict[int, str]) -> List[Dict[str, Any]]:
        ...
        for page_num, text in pages_dict.items():
            ...
            queries.extend(self._check_captions(page_num, text))
            queries.extend(self._check_section_references(page_num, text))
            queries.extend(self._check_date_consistency(page_num, text))
...
class AdvancedIntelligenceLayers:
    """
    Orchestrates all 4 dimensions of advanced intelligence
    """
    
    def __init__(self):
        print("✅ Advanced Intelligence Layers initialized (4 Dimensions)")
        self.contradiction_detector = ContradictionDetector()
        self.identity_tracker = EntityIdentityTracker()
        self.purpose_interrogator = PurposeInterrogator()
        self.hygiene_scanner = DraftingHygieneScanner()
    
    def analyze(self, pages_dict: Dict[int, str]) -> Dict[str, Any]:
        """
        ...
        Returns:
            Dict containing results from all dimensions
        """
        print(f"\n🧠 Running Advanced Intelligence Analysis ({len(pages_dict)} pages)...")
        results = {}
        
        # Dimension 1: Contradictions
        print(f"[Dimension 1/4] Detecting Contradictions...")
        ...
        print(f"   ✓ Found {len(results['contradictions'])} contradictions")
        
        # Dimension 2: Entity Identity
        print(f"\n[Dimension 2/4] Tracking Entity Identity...")
        self.identity_tracker.track_entities(pages_dict)
        results['entity_identity'] = self.identity_tracker.detect_inconsistencies()
        print(f"   ✓ Detected {len(results['entity_identity'])} identity inconsistencies")
