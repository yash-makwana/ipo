...
class ContentReviewEngine:
    ...
    def __init__(self, doc_data, chapter_name="Business Overview"):
        ...
        # V30: Context-Aware Capability
        self.context_gen = get_context_aware_generator()
    ...
    def generate_queries(self, pages_text, forensic_enabled=True):
        ...
        # 0. V30: INJECT CONTEXT-AWARE QUERIES (High Priority)
        try:
             # Extract structured context from pages
             print(f"[*] V30: Extracting deep context for Advanced Generators...")
             context = self.content_extractor.extract_complete_context(pages_text)
             
             # Generate specific queries
             adv_results = self.context_gen.generate_all_queries(context, pages_text)
             
             # Convert to Engine 3 format
             injected_count = 0
             for category, q_list in adv_results.items():
                 for q in q_list:
                     obs = q.get('observation', "").strip()
                     if not obs: continue
                     
                     # Map fields to Engine 3 structure
                     self.queries.append({
                         "page": q.get('page', "1"),
                         "text": obs + " Provide revised draft.",
                         "type": "TYPE_CONTEXT_AWARE", 
                         "issue_id": q.get('issue_id', "ADVANCED_QUERY"),
                         "severity": q.get('severity', "Material"),
                         "category": category.title()
                     })
                     injected_count += 1
             print(f"[*] V30: Injected {injected_count} context-aware queries")
        except Exception as e:
        ...
        # 5. STAGE 4: WRITING
        self._write_queries()
        before_count = len(self.queries)
        self.queries = self._deduplicate_queries(self.queries)
        print(f"[*] Deduplication: {before_count} → {len(self.queries)} queries")
        return self.queries
    ...
    def _deduplicate_queries(self, queries):
        """Prevent asking same thing 8-10 times"""
        
        # Remove exact duplicates
        unique_texts = {}
        for query in queries:
            text_key = ' '.join(query['text'].strip().lower().split())
            if text_key not in unique_texts:
                unique_texts[text_key] = query
        
        queries = list(unique_texts.values())
        
        # Merge similar queries (>80% similarity)
        queries = self._merge_similar_queries(queries)
        
        # Enforce topic budgets
        queries = self._enforce_topic_budgets(queries)
        
        return queries
    
    def _merge_similar_queries(self, queries):
        """Merge queries >80% similar"""
        merged = []
        skip_indices = set()
        
        for i, query_a in enumerate(queries):
            if i in skip_indices:
                continue
            
            similar_group = [query_a]
            ...
            for j in range(i + 1, len(queries)):
                ...
                query_b = queries[j]
                if self._calculate_similarity(query_a['text'], query_b['text']) > 0.80:
                    similar_group.append(query_b)
                    skip_indices.add(j)
            
            # Use longest query from similar group
            best = max(similar_group, key=lambda q: len(q['text']))
            merged.append(best)
        
        return merged
    
    def _calculate_similarity(self, text_a, text_b):
        """Calculate text similarity"""
        matcher = difflib.SequenceMatcher(None, text_a.lower(), text_b.lower())
        return matcher.ratio()
    
    def _enforce_topic_budgets(self, queries):
        """Max 1 query per specific topic"""
        
        BUDGETS = {
            'custom machinery': 1,  # ← FIXES YOUR OVER-HIT
            'machinery': 1,
            'r&d': 2,
            'supplier': 1,
        ...
        for query in queries:
            topic = self._identify_query_topic(query['text'])
            topic_groups[topic].append(query)
        
        filtered = []
        for topic, topic_queries in topic_groups.items():
            budget = BUDGETS.get(topic, 3)
            
            if len(topic_queries) <= budget:
                filtered.extend(topic_queries)
            else:
                # Keep best queries (by severity)
                sorted_q = sorted(
                    topic_queries,
                    key=lambda q: (
                        {'Critical': 3, 'Major': 2, 'Material': 1}.get(q.get('severity', 'Material'), 0),
                        len(q['text'])
                    ),
                    reverse=True
                )
                filtered.extend(sorted_q[:budget])
        
        return filtered
    
    def _identify_query_topic(self, text):
        """Identify query topic"""
        t = text.lower()
        
        if 'custom machinery' in t or 'bespoke' in t:
            return 'custom machinery'
        elif 'machinery' in t:
            return 'machinery'
        elif 'r&d' in t or 'research' in t:
            return 'r&d'
        elif 'supplier' in t or 'vendor' in t:
            return 'supplier'
        elif 'customer concentration' in t:
